...
#define CONFIG_MV88E61XX_FIXED_PORTS 0x70
#endif
			       ...
			       u16 val)
{
	...
	res = mdio_bus->write(mdio_bus, smi_addr, MDIO_DEVAD_NONE,
				SMI_DATA_REG, val);
  /* printf("junote regwrite smi_addr=%x,reg=%x,data=%x\n",smi_addr,SMI_DATA_REG,val); */
	if (res < 0)
		return res;

	/* Issue the write command */
  /* printf("junote regwrite smi_addr=%x,reg=%x,cmddata=%x\n",smi_addr,SMI_CMD_REG,smi_cmd_write(dev,reg)); */
	res = mdio_bus->write(mdio_bus, smi_addr, MDIO_DEVAD_NONE, SMI_CMD_REG,
...
static void mv88e61xx_phy_config(struct phy_device *phydev)
{
	int res;
	int i;
	int ret = -1;
  ...
  printf("junote mv88e61xx_phy_config\n");
    ...
    mv88e61xx_phy_write(phydev,1,0,0x3100);
    mv88e61xx_phy_write(phydev,2,0,0x3100);

    mv88e61xx_reg_write(phydev, 0x18, 0x1, 0x403e);
    mv88e61xx_reg_write(phydev, 0x19, 0x1, 0x403e);
    mv88e61xx_reg_write(phydev, 0x1a, 0x1, 0x403e);


    mv88e61xx_reg_write(phydev, 0x10, 0x4, 0x7f);
    mv88e61xx_reg_write(phydev, 0x11, 0x4, 0x7f);
    mv88e61xx_reg_write(phydev, 0x12, 0x4, 0x7f);
    mv88e61xx_reg_write(phydev, 0x13, 0x4, 0x7f);
